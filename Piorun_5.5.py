...
"""
Generator dokumentacji Krok po Kroku - Piorun 5.5
Wersja z PySide6 (Qt for Python)
Poprawiona wersja – stały folder roboczy, zapamiętywanie ścieżki w podglądzie,
automatyczne odtwarzanie brakujących obrazów z danych JSON.
"""
...
class ImageResizerThread(QThread):
    """Wątek do przetwarzania obrazów – zapisuje pliki w folderze roboczym"""
    progress = Signal(int, int, str)
    finished = Signal(list)

    def __init__(self, image_paths, target_dir):
        super().__init__()
        ...
        try:
            with Image.open(input_path) as img:
                ...
                if file_size <= 800 * 1024:
                    ...
                    output_path = os.path.join(self.target_dir, f"{unique_id}{ext}")
                ...
                timestamp = int(datetime.now().timestamp() * 1000)
                unique_id = f"{timestamp}_{hashlib.md5(input_path.encode()).hexdigest()[:8]}"
                output_path = os.path.join(self.target_dir, f"{unique_id}.jpg")
...
class GeneratorDokumentow(QMainWindow):
    def __init__(self):
        ...
        self.ilustracje = []          # lista ścieżek do plików obrazów
        self.opisy_krokow = {}         # słownik {index: {'nazwa': ..., 'opis': ...}}
        self.loaded_images_data = {}    # słownik {ścieżka: dane_base64} – do odtwarzania brakujących plików
        self.sciezka_doc = "dokumentacja_kpk.docx"
        self.aktualny_wybrany_krok = None

        # Stały folder roboczy w katalogu użytkownika
        self.work_dir = os.path.join(os.path.expanduser("~"), "Piorun_workdir")
        os.makedirs(self.work_dir, exist_ok=True)

        # Podfolder dla tymczasowych plików edycji w Paincie
        self.paint_temp_dir = os.path.join(self.work_dir, "paint_edit")
        os.makedirs(self.paint_temp_dir, exist_ok=True)
        ...
        # Flaga blokująca wielokrotne uruchamianie wątków
        ...
        # Sprawdź autozapis przy starcie
        self.sprawdz_autozapis_przy_starcie()

    def closeEvent(self, event):
        """Bezpieczne zamknięcie – usuwamy tylko tymczasowe pliki edycji."""
        reply = QMessageBox.question(self, 'Wyjście', "Czy na pewno chcesz zamknąć program?",
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.No)

        if reply == QMessageBox.Yes:
            try:
                if os.path.exists(self.paint_temp_dir):
                    shutil.rmtree(self.paint_temp_dir)
            except Exception as e:
                print(f"Błąd podczas usuwania {self.paint_temp_dir}: {e}")
            event.accept()
        else:
            event.ignore()

    def sprawdz_autozapis_przy_starcie(self):
        try:
            pliki = []
            for plik in os.listdir(self.autosave_dir):
                if plik.endswith('.json') and ('_' in plik):
            ...
            pliki.sort(key=lambda x: x[1], reverse=True)
            najnowszy = pliki[0][0]

            odpowiedz = QMessageBox.question(
                self,
                "Przywracanie autozapisu",
                f"Znaleziono ostatni autozapis z {datetime.fromtimestamp(pliki[0][1]).strftime('%Y-%m-%d %H:%M:%S')}.\nCzy chcesz go przywrócić?",
                QMessageBox.Yes | QMessageBox.No
            )
            if odpowiedz == QMessageBox.Yes:
                self.wczytaj_projekt_z_sciezki(najnowszy)
        except Exception as e:
            print(f"Błąd podczas sprawdzania autozapisów: {e}")
...
Machine: [Enter machine or device name here]""",
    ...
    def t(self, klucz):
        return self.tlumaczenia.get(self.jezyk, {}).get(klucz, f"[{klucz}]")
    ...
    def setup_ui(self):
        ...
        self.autor_edit = QLineEdit("Dorota Zaręba")
        ...
        self.generate_btn.setFixedWidth(240)
    ...
    def setup_menu(self):
        ...
        enable_autosave_action.triggered.connect(lambda checked: setattr(self, 'autosave_enabled', checked))
    ...
    def dodaj_ilustracje(self):
        if self.processing:
            QMessageBox.information(self, "Informacja", "Trwa przetwarzanie obrazów, proszę czekać.")
            return
        ...
        self.resizer_thread = ImageResizerThread(files, self.work_dir)
        self.resizer_thread.progress.connect(lambda cur, total, name: progress.setValue(cur))
        self.resizer_thread.finished.connect(lambda paths: self.on_images_processed(paths, progress))
    ...
    def on_images_processed(self, processed_paths, progress):
        ...
        for i, plik in enumerate(processed_paths):
            ...
            self.steps_list.add_image_item(plik, index + 1, nazwa_kroku)
    ...
    def wymien_ilustracje(self):
        ...
        if current_row < 0:
            QMessageBox.warning(self, self.t("ostrzeżenie"), self.t("najpierw_wybierz_ilustracje_do_wymiany"))
            return
        ...
        if plik:
            ...
            self.steps_list.update_item(index, przetworzony_plik, index + 1, zachowana_nazwa)

            if index not in self.opisy_krokow:
                self.opisy_krokow[index] = {'nazwa': zachowana_nazwa, 'opis': self.t("domyslny_szablon")}
            else:
                self.opisy_krokow[index]['nazwa'] = zachowana_nazwa

            self.steps_list.setCurrentRow(index)
            self.pokaz_podglad()
            QMessageBox.information(self, self.t("sukces"), self.t("ilustracja_zostala_wymieniona"))
        ...
        try:
            with Image.open(input_path) as img:
                ...
                if file_size <= 800 * 1024:
                    ...
                    unique_id = f"{timestamp}_{hashlib.md5(input_path.encode()).hexdigest()[:8]}"
                    ext = os.path.splitext(input_path)[1] or '.jpg'
                    output_path = os.path.join(self.work_dir, f"{unique_id}{ext}")
                    shutil.copy2(input_path, output_path)
                    return output_path
                ...
                timestamp = int(datetime.now().timestamp() * 1000)
                unique_id = f"{timestamp}_{hashlib.md5(input_path.encode()).hexdigest()[:8]}"
                output_path = os.path.join(self.work_dir, f"{unique_id}.jpg")
                img.save(output_path, 'JPEG', quality=85, optimize=True)
                return output_path
    ...
    def usun_ilustracje(self):
        ...
        self.steps_list.takeItem(index)
    ...
    def pokaz_podglad(self, item=None):
        ...
        if current_row >= 0 and current_row < len(self.ilustracje):
            ...
            # Jeśli plik nie istnieje, spróbuj odtworzyć z zapamiętanych danych
            if not os.path.exists(sciezka_obrazu):
                self._przywroc_obraz_z_danych(sciezka_obrazu)

            if os.path.exists(sciezka_obrazu):
                self.preview_widget.set_image(sciezka_obrazu)
            else:
                QMessageBox.warning(self, "Uwaga", f"Nie można odtworzyć pliku obrazu:\n{sciezka_obrazu}")
                self.preview_widget.set_image(None)
    ...
    def _przywroc_obraz_z_danych(self, sciezka):
        """Próbuje odtworzyć plik obrazu z zapamiętanych danych base64"""
        if sciezka in self.loaded_images_data:
            try:
                dane_base64 = self.loaded_images_data[sciezka]
                obraz_binary = base64.b64decode(dane_base64)
                with open(sciezka, 'wb') as f:
                    f.write(obraz_binary)
                print(f"Przywrócono plik: {sciezka}")
            except Exception as e:
                print(f"Błąd przywracania pliku {sciezka}: {e}")
    ...
    def zapisz_opis_kroku(self):
        if self.aktualny_wybrany_krok is None:
            QMessageBox.warning(self, self.t("ostrzeżenie"), self.t("najpierw_wybierz_krok"))
            return
        self.zapisz_stan()
        ...
        self.opisy_krokow[index] = {'nazwa': nazwa, 'opis': opis}
        step_number = index + 1
        if 0 <= index < self.steps_list.count():
            sciezka = self.ilustracje[index]
            self.steps_list.update_item(index, sciezka, step_number, nazwa)
        self.aktualizuj_info_o_kroku()
        QMessageBox.information(self, self.t("sukces"), self.t("opis_kroku_zostal_zapisany"))
    ...
    def generuj_i_otworz_dokument(self):
        if not self.ilustracje:
            QMessageBox.warning(self, self.t("ostrzeżenie"), self.t("dodaj_przynajmniej_jedna_ilustracje"))
            return
        if not self.kod_edit.text() or not self.nazwa_edit.text():
            QMessageBox.warning(self, self.t("ostrzeżenie"), self.t("wprowadz_kod_i_nazwe_dokumentu"))
            return
        ...
        try:
            ...
            doc.add_paragraph().add_run(f"{self.t('liczba_krokow')} {len(self.ilustracje)}").bold = True
            doc.add_page_break()
            progress.setValue(4)
            ...
            if os.path.exists(self.sciezka_doc):
                ...
                QMessageBox.warning(self, self.t("ostrzeżenie"), self.t("dokument_wygenerowany_ale_nie_otwarty"))
    ...
    def edytuj_w_paincie(self):
        current_row = self.steps_list.currentRow()
        if current_row < 0:
            QMessageBox.warning(self, self.t("ostrzeżenie"), self.t("najpierw_wybierz_obraz"))
            return

        index = current_row
        if index >= len(self.ilustracje):
            QMessageBox.warning(self, self.t("błąd"), "Indeks poza zakresem listy obrazów!")
            return
        ...
        if not os.path.exists(sciezka_obrazu):
            ...
            if not os.path.exists(sciezka_obrazu):
                QMessageBox.warning(self, self.t("błąd"),
                                  f"Obraz nie istnieje i nie można go odtworzyć:\n{sciezka_obrazu}")
                return
            ...
            if sys.platform == "win32":
                ...
                try:
                    with Image.open(sciezka_obrazu) as img:
                        if img.mode in ('RGBA', 'LA', 'P'):
                            ...
                            img.convert('RGB').save(kopia_obrazu, 'PNG')
                except Exception as e:
                    shutil.copy2(sciezka_obrazu, kopia_obrazu)

                if not os.path.exists(kopia_obrazu):
                    QMessageBox.warning(self, self.t("błąd"), "Nie udało się utworzyć kopii obrazu do edycji!")
                    return

                process = subprocess.Popen(["mspaint.exe", kopia_obrazu])
                ...
                if result == QMessageBox.Yes:
                    ...
                    if os.path.exists(kopia_obrazu) and os.path.getsize(kopia_obrazu) > 100:
                        ...
                        # Kopia zapasowa oryginału
                        if self.work_dir not in sciezka_obrazu:
                            dir_path = os.path.dirname(sciezka_obrazu)
                            backup_sciezka = os.path.join(
                                dir_path,
                                f"{nazwa_bez_ext}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                            )
                        ...
                        try:
                            ...
                            self.steps_list.setCurrentRow(index)
                            self.pokaz_podglad()
                            QMessageBox.information(self, self.t("sukces"), self.t("obraz_zaktualizowany"))
                        except Exception as e:
                            if 'backup_sciezka' in locals() and os.path.exists(backup_sciezka):
                                shutil.copy2(backup_sciezka, final_sciezka)
                            QMessageBox.warning(self, self.t("błąd"), f"Błąd podczas zapisywania edytowanego obrazu: {str(e)}")
                    else:
                        QMessageBox.warning(self, self.t("ostrzeżenie"), "Edytowany plik jest pusty lub nie istnieje!")

                # Nie usuwamy pliku tymczasowego od razu – zostanie usunięty przy zamykaniu programu
                # (lub możemy go usunąć, ale to bezpieczniej zostawić)

            else:
                QMessageBox.warning(self, self.t("ostrzeżenie"), self.t("funkcja_dostepna_tylko_windows"))

        except Exception as e:
            QMessageBox.critical(self, self.t("błąd"), f"{self.t('błąd_otwierania_paint')}: {e}")
    ...
    def zapisz_projekt(self):
        if not self.ilustracje:
            QMessageBox.warning(self, self.t("ostrzeżenie"), self.t("brak_danych_do_zapisania"))
            return
                    ...
                    try:
                        if os.path.exists(sciezka_img):
                            with open(sciezka_img, 'rb') as f:
                                ...
                                obrazy_base64[str(i)] = {'nazwa': nazwa_pliku, 'dane': obraz_base64}
                        else:
                            # Jeśli plik nie istnieje, próbujemy użyć zapamiętanych danych
                            if sciezka_img in self.loaded_images_data:
                                obrazy_base64[str(i)] = {
                                    'nazwa': os.path.basename(sciezka_img),
                                    'dane': self.loaded_images_data[sciezka_img]
                                }
                            else:
                                QMessageBox.warning(self, self.t("ostrzeżenie"),
                                                  f"Plik {sciezka_img} nie istnieje i nie ma zapamiętanych danych – pomijam.")
                    except Exception as e:
                        QMessageBox.warning(self, self.t("ostrzeżenie"),
                                          f"{self.t('błąd_odczytu_obrazu')} {os.path.basename(sciezka_img)}: {e}")

                opisy_krokow_str_keys = {str(k): v for k, v in self.opisy_krokow.items()}
    ...
    def wczytaj_projekt(self):
        ...
        if sciezka:
            self.wczytaj_projekt_z_sciezki(sciezka)

    def wczytaj_projekt_z_sciezki(self, sciezka):
        try:
            ...
            for i in range(len(obrazy_base64)):
                ...
                if str_i in obrazy_base64:
                    ...
                    # Generujemy unikalną nazwę pliku w folderze roboczym
                    sciezka_obrazu = os.path.join(self.work_dir, f"loaded_{i}_{nazwa_pliku}")
                    self.ilustracje.append(sciezka_obrazu)
                    self.loaded_images_data[sciezka_obrazu] = dane_base64
                    ...
                    try:
                        ...
                        print(f"Błąd zapisu pliku {sciezka_obrazu}: {e}")
    ...
    def utworz_kopie_zapasowa(self):
        if not self.ilustracje:
            QMessageBox.warning(self, self.t("ostrzeżenie"), "Brak danych do kopii zapasowej!")
            return
        ...
        try:
            ...
            for i, sciezka in enumerate(self.ilustracje):
                if os.path.exists(sciezka):
                    ...
                    # Jeśli plik nie istnieje, spróbuj odtworzyć z danych
                    if sciezka in self.loaded_images_data:
                        try:
                            dane = base64.b64decode(self.loaded_images_data[sciezka])
                            dest_path = os.path.join(backup_dir, f"krok_{i+1}_{os.path.basename(sciezka)}")
                            with open(dest_path, 'wb') as f:
                                f.write(dane)
                        except Exception as e:
                            print(f"Nie udało się zapisać pliku {sciezka} do kopii: {e}")
                ...
                try:
                    if os.path.exists(sciezka_img):
                        with open(sciezka_img, 'rb') as f:
                            ...
                            obrazy_base64[str(i)] = {'nazwa': nazwa_pliku_img, 'dane': obraz_base64}
                    else:
                        if sciezka_img in self.loaded_images_data:
                            obrazy_base64[str(i)] = {
                                'nazwa': os.path.basename(sciezka_img),
